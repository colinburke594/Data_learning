...
import CleanDatasetLibrary from "./components/CleanDatasetLibrary";
import CourseLibrary from "./components/CourseLibrary";
import { DOMAINS } from "./data/roadmapData";
import { DATASETS } from "./data/datasetData";
import { CLEAN_DATASETS } from "./data/cleanDatasetData";
import { COURSES } from "./data/courseData";

const TABS = [
  { id: "roadmap",      label: "Skill Roadmap",    icon: "🗺️" },
  { id: "clean",        label: "Clean Datasets",   icon: "✨" },
  { id: "dirty",        label: "Dirty Datasets",   icon: "🧹" },
...
function computeOverallProgress() {
  ...
  // Clean Datasets
  let cleanDatasetDone = 0;
  try {
    const comp = JSON.parse(localStorage.getItem("clean_dataset_completed") || "{}");
    cleanDatasetDone = Object.values(comp).filter(Boolean).length;
  } catch {}
  ...
  const total = roadmapTotal + DATASETS.length + CLEAN_DATASETS.length + COURSES.length;
  const done = roadmapDone + datasetDone + cleanDatasetDone + courseDone;
...
export default function App() {
  ...
  useEffect(() => {
    ...
    localStorage.setItem = (key, value) => {
      ...
      if (["roadmap_checked", "dataset_completed", "clean_dataset_completed", "course_completed"].includes(key)) {
        // Debounce slightly
        setTimeout(() => {
          setProgress(computeOverallProgress());
          setSaveFlash(true);
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-500 hidden sm:block">
                  Roadmap · Clean Data · Dirty Data · Courses
                </p>
              </div>
            </div>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {(() => {
            ...
            const cleanDone = (() => { try { return Object.values(JSON.parse(localStorage.getItem("clean_dataset_completed") || "{}")).filter(Boolean).length; } catch { return 0; } })();
            ...
            return [
              ...
              { label: "✨ Clean", done: cleanDone, total: CLEAN_DATASETS.length, tab: "clean" },
              { label: "🧹 Dirty", done: datasetDone, total: DATASETS.length, tab: "dirty" },
        ...
        <div role="tabpanel">
          ...
          {activeTab === "clean"    && <CleanDatasetLibrary />}
          {activeTab === "dirty"    && <DatasetLibrary />}
